...
def get_layer_names(model_id):
    ...
    layer_drop_names = [x for x in modul_names if len(re.findall(r'transformer.dropout$', x)) > 0]
    layer_output_names = [x for x in modul_names if len(re.findall(r'transformer.h.\d+$', x)) > 0]
