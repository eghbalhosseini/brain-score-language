...
def DsParametric_fMRI_auditory():
    return load_DsParametric_fMRI_V2(atlas='auditory')

def DsParametric_fMRI_language():
    return load_DsParametric_fMRI_V2(atlas='language',threshold=70)

def DsParametric_fMRI_visual():
    return load_DsParametric_fMRI_V2(atlas='visual',threshold=70)


def upload_DsParametric_fMRI(atlas):
    assembly = load_DsParametric_fMRI_V2(atlas=atlas)
    upload_data_assembly(assembly,
                         assembly_identifier=f"DsParametric_fMRI.{atlas}")

def load_DsParametric_fMRI_extended(threshold=80):
    data_dir=Path(f'/om/weka/evlab/ehoseini//MyData/fmri_DNN/outputs/DsParametricfMRI_all_subs_5_thresh-{threshold}_analyzed.pkl')
    atlas_dir=Path(f'/rdma/vast-rdma/vast/evlab/ehoseini/MyData/brain-score-language/dataset//DsParametric_fMRI_atlas_thr_{threshold}.pkl')
    if atlas_dir.exists():
        atlas_data=pd.read_pickle(atlas_dir.__str__())
    else:
        ...
        for id, atlas_id in enumerate(['language','auditory','visual']):
            ...
            sentences=data_atlas.stim_value.values
            #fix issue with sess_id
            # average over repeats
            data_atlas=data_atlas.mean('repeat')
            ...
            assembly=NeuroidAssembly(data_atlas.values,coords={
                'experiment':('presentation',np.repeat('DsParametric_fMRI',data_atlas.shape[0])),
                'stimulus_num':('presentation',data_atlas.stim_name.values),
                'stimulus_id': ('presentation', data_atlas.stim_id.values),
                'stim_group': ('presentation', data_atlas.stim_group.values),
                'stim_group_id': ('presentation', data_atlas.stim_group_id.values),
                'sentence':('presentation',data_atlas.stim_value.values),
                'stimulus': ('presentation', data_atlas.stim_value.values),
            ...
            assembly.attrs['identifier'] = f"DsParametric_fMRI.all.{atlas_id}_top_{threshold}_V2"
            atlas_data[atlas_id]=assembly

        pd.to_pickle(atlas_data,atlas_dir.__str__())
    return atlas_data
# def load_DsParametric_fMRI_for_RSA_extended(threshold=80):
...
#
#         data_atlas=xr.concat(all_sub_sessions,'voxels')
#         assert (data_atlas.shape[2] == np.sum([x.shape[2] for x in all_sub_sessions]))
#         roi_list = data_atlas[name_pair[atlas_id]+'_rois'].values
#         atlas_list=np.repeat(atlas_id,data_atlas.shape[2])
#         assembly=NeuroidAssembly(data_atlas.values,coords={
#             'experiment':('presentation',np.repeat('DsParametric_fMRI',data_atlas.shape[1])),
#             'stimulus_num':('presentation',data_atlas.stim_name.values),
#             'stimulus_id': ('presentation', data_atlas.stim_id.values),
...
#             'TR_duration': ('presentation', data_atlas.TR_duration.values),
#             'subject': ('neuroid',data_atlas.subject.values),
#             'sess_id': ('neuroid', data_atlas.sess_id.values),
#             'tval_lang': ('neuroid', data_atlas.tval_lang.values),
#             'neuroid_id':('neuroid',data_atlas.neuroid_id.values),
#             'voxel_num':('neuroid',data_atlas.voxel_num.values),
#             'repetition_corr_ratio': ('neuroid', data_atlas.repetition_corr_ratio.values),
#             'repetition_corr': ('neuroid', data_atlas.repetition_corr.values),
#             'roi':('neuroid',roi_list),
#             'atlas':('neuroid',atlas_list),
#             'sess_repeat':('repeat',data_atlas.sess_repeat.values),
#             #'abs_repeat': ('repeat', data_atlas.abs_repeat.values)
#         },dims=['repeat','presentation','neuroid'])
#         assembly = assembly.sortby('stimulus_id')
#         assembly.attrs['identifier'] = f"DsParametric_fMRI_RSA.all.{atlas_id}_top_{threshold}_V2"
#         atlas_data[atlas_id]=assembly
#     return atlas_data

def load_DsParametric_fMRI_V2(atlas=None,threshold=70,vox_reliability=None,vox_corr=None):
    atlas_data=load_DsParametric_fMRI_extended(threshold)
    ## preset for voxel reliability, and correlation
    if vox_reliability is None:
...
# def load_DsParametric_fMRI_RSA_V2(atlas=None,group_id='train',threshold=90):
#     atlas_list=load_DsParametric_fMRI_for_RSA_extended(threshold)
#     ## preset for voxel reliability, and correlation
...
if __name__ == '__main__':
    ...
    #upload_DsParametric_fMRI(atlas='language')
    #upload_DsParametric_fMRI(atlas='auditory')
    #upload_DsParametric_fMRI(atlas='visual')